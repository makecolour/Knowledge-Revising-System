        ...
        <div class="" style="display: flex; flex-direction: column">
            <div class="">
                ...
                <div style="display: flex; flex-direction: row; justify-content: space-between">
                    ...
                    <div>
                        <a href="/subject/adding" class="btn btn-secondary">Add</a>
                    </div>
                </div>
                ...
                <div class="table-responsive">
                    <table class="table table-striped">
                        ...
                        <c:forEach var="list" items="${courseList}">
                            <tr>
                                <td>${list.subject.id}</td>
                                <td>${list.subject.name}</td>
                                <td>
                                    <a href="<c:url value='/lessons-list/${list.id}' />">${list.subject.code}</a>
                                </td>
                                <td>${list.subject.role.speciality}</td>
                                <td>${list.subject.subjectManager[0].user.fullName}</td>
                                <td>${list.subject.status ? "Active" : "Inactive"}</td>
                                <td>
                                    <form action="/subject/${list.subject.id}" method="get" style="display: inline;">
                    ...
                    <div aria-label="Page navigation">
                        <ul class="pagination">
                            <c:if test="${currentPage > 0}">
                                <li class="page-item">
                                    <a class="page-link"
                                       href="${pageContext.request.contextPath}/subject/search?courseName=${param.courseName}&subjectId=${param.subjectId}&status=${param.status}&page=${currentPage-1}&size=${9}">Previous</a>
                                </li>
                            </c:if>
                            <c:if test="${totalPages > 0}">
                                <c:forEach var="i" begin="0" end="${totalPages - 1}">
                                    <li class="page-item ${i == currentPage ? 'active' : ''}">
                                        <a class="page-link"
                                           href="${pageContext.request.contextPath}/subject/search?courseName=${param.courseName}&subjectId=${param.subjectId}&status=${param.status}&page=${i}&size=${9}">${i + 1}</a>
                                    </li>
                                </c:forEach>
                            </c:if>
                            <c:if test="${currentPage < totalPages - 1}">
                                <li class="page-item">
                                    <a class="page-link"
                                       href="${pageContext.request.contextPath}/subject/search?courseName=${param.courseName}&subjectId=${param.subjectId}&status=${param.status}&page=${currentPage+1}&size=${9}">Next</a>
                                </li>
                            </c:if>
                        </ul>
                    </div>
